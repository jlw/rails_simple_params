...
describe RailsSimpleParams::Parameter do
  let(:name)    { 'foo' }
  let(:value)   { 'bar' }
  ...
  describe '#transform' do
    ...
    context 'with a block' do
      let(:value)   { 'BAR' }
      let(:options) { { transform: ->(n) { n.downcase } } } # rubocop:disable Style/SymbolProc
      ...
      context 'conditional block' do
        let(:value)   { false }
        let(:options) { { transform: ->(n) { n ? 'foo' : 'no foo' } } }
  ...
  describe '#validate' do
    ...
    before :each do
      allow(RailsSimpleParams::Validator).to receive(:new).and_return(validator)
      allow(validator).to receive(:validate!)
    end

    it 'passes self to Validator' do
      subject.validate
      expect(RailsSimpleParams::Validator).to have_received(:new).with(subject)
    end
